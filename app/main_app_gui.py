...
# Hardcoded POSE_CONNECTIONS based on MediaPipe's standard pose model
...
# FIX (MD §4): VIS_THRESH hoisted to module-level constant — was re-defined inside callback on every frame
VIS_THRESH = 0.5
...
BICEP_CURL_START_ANGLE = 150
BICEP_CURL_END_ANGLE = 50

SQUAT_START_ANGLE = 150
SQUAT_END_ANGLE = 100

PUSHUP_START_ANGLE = 150
PUSHUP_END_ANGLE = 100
...
# Voice feedback cooldown (seconds) — from MD §4
...
class App(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            self.form_classification_model = None
        ...
        class StartScreen(ctk.CTkFrame):
            def __init__(self, master, controller):
                ...
                start_button = ctk.CTkButton(
                    self, text="Start",
                    command=lambda: self.controller._show_frame_controller("ModeSelectionScreen")
                )
        ...
        class ModeSelectionScreen(ctk.CTkFrame):
            def __init__(self, master, controller):
                ...
                label.pack(pady=10, padx=10)
                gym_button = ctk.CTkButton(
                    self, text="Gym Mode",
                    command=lambda: self.controller._show_frame_controller("ExerciseSelectionScreen", mode="Gym")
                )
                gym_button.pack(pady=5)
                home_button = ctk.CTkButton(
                    self, text="Home Mode",
                    command=lambda: self.controller._show_frame_controller("ExerciseSelectionScreen", mode="Home")
                )
                home_button.pack(pady=5)
                back_button = ctk.CTkButton(
                    self, text="Back",
                    command=lambda: self.controller._show_frame_controller("StartScreen")
                )
        ...
        class ExerciseSelectionScreen(ctk.CTkFrame):
            ...
            def update_content(self, mode):
                ...
                self.mode_exercise_label = ctk.CTkLabel(
                    self, text=f"Select Exercise for {self.selected_mode} Mode"
                )
                self.mode_exercise_label.pack(pady=10, padx=10)

                exercises = EXERCISES_BY_MODE.get(self.selected_mode, [])
                if not exercises:
                    no_exercises_label = ctk.CTkLabel(
                        self.exercise_frame, text="No exercises defined for this mode."
                    )
                ...
                for exercise in exercises:
                    exercise_button = ctk.CTkButton(
                        self.exercise_frame, text=exercise,
                        command=lambda e=exercise: self.controller._show_frame_controller(
                            "InputSourceScreen", mode=self.selected_mode, exercise=e
                        )
                    )
        ...
        class InputSourceScreen(ctk.CTkFrame):
            def __init__(self, master, controller):
                ...
                webcam_button = ctk.CTkButton(
                    self, text="Online Training (Webcam)",
                    command=lambda: self.controller._show_frame_controller(
                        "LiveSessionScreen",
                ...
                upload_button = ctk.CTkButton(
                    self, text="Upload Video File",
                    command=lambda: self._select_video_file()
                )
                ...
                back_button = ctk.CTkButton(
                    self, text="Back",
                    command=lambda: self.controller._show_frame_controller(
                        "ExerciseSelectionScreen", mode=self.selected_mode
                    )
                )
                back_button.pack(pady=10, side="bottom")
            ...
            def update_content(self, mode, exercise):
                ...
                self.dynamic_label = ctk.CTkLabel(
                    self, text=f"Input for {self.selected_mode} - {self.selected_exercise}"
                )
            ...
            def _select_video_file(self):
                ...
                if video_path:
                    ...
                    self.controller._show_frame_controller(
                        "LiveSessionScreen",
                        mode=self.selected_mode,
                        exercise=self.selected_exercise,
        ...
        class LiveSessionScreen(ctk.CTkFrame):
            def __init__(self, master, controller):
                ...
                # FIX (MD §4): Initialize feedback_l / feedback_r to prevent AttributeError
                # on first gym-mode frame where only one side is visible
                self.feedback_l = "..."
                self.feedback_r = "..."
            ...
            def update_content(self, mode, exercise, input_source, video_path=None):
                ...
                self.feedback_label.configure(text="Feedback: Preparing...")
                self.reps_label.configure(text="L Reps: 0 | R Reps: 0")
                self.rep_count_left = 0
                self.rep_count_right = 0
                self.rep_count = 0
                self.feedback_text_rep = ""
                # FIX (MD §4): Initialize feedback_l / feedback_r on every new session
                self.feedback_l = "..."
                self.feedback_r = "..."
                ...
                # Reset voice throttle for new session
                self._last_spoken = ""
                self._last_speak_time = 0.0
            ...
            def _start_session(self):
                ...
                def result_callback(result: PoseLandmarkerResult, output_image: mp.Image, timestamp_ms: int):
                    ...
                    if result.pose_landmarks:
                        ...
                        # Extract landmarks and visibility scores
                        ...
                        l_hi = [lm[23].x, lm[23].y]; l_kn = [lm[25].x, lm[25].y]; l_an = [lm[27].x, lm[27].y]
                        r_hi = [lm[24].x, lm[24].y]; r_kn = [lm[26].x, lm[26].y]; r_an = [lm[28].x, lm[28].y]
                        ...
                        if self.selected_exercise == "Bicep Curl":
                        ...
                        elif self.selected_exercise == "Lat Pulldown/Row":
                            s_th, e_th, f_up, f_dn = LAT_PULLDOWN_START_ANGLE, LAT_PULLDOWN_END_ANGLE, "Pull", "Extend"
                        elif self.selected_exercise == "Chest Press":
                            s_th, e_th, f_up, f_dn = CHEST_PRESS_START_ANGLE, CHEST_PRESS_END_ANGLE, "Press", "Lower"
                        elif self.selected_exercise == "Shoulder Press":
                            s_th, e_th, f_up, f_dn = SHOULDER_PRESS_START_ANGLE, SHOULDER_PRESS_END_ANGLE, "Press Up", "Lower"
                        elif self.selected_exercise in ["Squats", "Lunges"]:
                            ...
                            s_th, e_th, f_up, f_dn = 150, 100, "Up", "Lower"

                        # Rep counting FSM logic
                                ...
                                if vis_combined > VIS_THRESH:
                                    if angle > s_th:
                                        ...
                                        if self.is_flexed:
                                            ...
                                            self.feedback_text_rep = f_dn
                                    elif angle < e_th:
                                        if self.is_extended:
                                            self.is_flexed = True
                                            self.feedback_text_rep = f_up
                                        else:
                                            self.feedback_text_rep = f_dn
                                    else:
                                        self.feedback_text_rep = "..."
                                ...
                                if vis_l > VIS_THRESH:
                                    if angle_l > s_th:
                                        ...
                                        if self.is_flexed_left:
                                            ...
                                            self.feedback_l = f_dn
                                    elif angle_l < e_th:
                                        if self.is_extended_left:
                                            self.is_flexed_left = True
                                            self.feedback_l = f_up
                                        else:
                                            self.feedback_l = f_dn
                                    else:
                                        self.feedback_l = "..."
                                else:
                                    self.feedback_l = "Hidden"
                                ...
                                if vis_r > VIS_THRESH:
                                    if angle_r > s_th:
                                        ...
                                        if self.is_flexed_right:
                                            ...
                                            self.feedback_r = f_dn
                                    elif angle_r < e_th:
                                        if self.is_extended_right:
                                            self.is_flexed_right = True
                                            self.feedback_r = f_up
                                        else:
                                            self.feedback_r = f_dn
                                    else:
                                        self.feedback_r = "..."
                                else:
                                    self.feedback_r = "Hidden"
                        ...
                        # Final feedback text update
                        self.feedback_text = f"{self.feedback_text_rep} | L: {int(angle_l)}° R: {int(angle_r)}° | Form: Classifying..."

                        # FIX (MD §4): Wire voice feedback — throttled TTS in a daemon thread
                        # so it never blocks the pose callback or the GUI thread
                        now = time.time()
                        if (self.feedback_text_rep != self._last_spoken
                                and now - self._last_speak_time > VOICE_COOLDOWN_SECONDS
                                and self.feedback_text_rep not in ("...", "Hidden", "")):
                            self._last_spoken = self.feedback_text_rep
                            self._last_speak_time = now
                            threading.Thread(
                                target=lambda t=self.feedback_text_rep: (
                                    self.controller.engine.say(t),
                                    self.controller.engine.runAndWait()
                                ),
                                daemon=True
                            ).start()
                        ...
                        self.landmark_buffer.append(
                            np.full((NUM_LANDMARKS, NUM_FEATURES), np.nan, dtype=np.float32).tolist()
                        )
            ...
            def _update_frame(self):
                ...
                #    in the mediapipe.tasks API and raises AttributeError at runtime.
                ...
                # 3. Flip for selfie view (flips person + drawn landmarks together)
                if self.input_source == "webcam":
                    frame = cv2.flip(frame, 1)

                # 4. Resize to fit the display area
                ...
                # 5. Convert to PIL/Tkinter and display
                img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
                imgtk = ImageTk.PhotoImage(image=img)
                self.video_label.imgtk = imgtk
                self.video_label.configure(image=imgtk)

                # 6. Update info labels
                ...
                self.frame_update_id = self.after(40, self._update_frame)  # ~25 FPS
            ...
            def _end_session(self):
                ...
                self.video_label.configure(image=None, text="Video Session Ended")
        ...
        for F in (StartScreen, ModeSelectionScreen, ExerciseSelectionScreen, InputSourceScreen, LiveSessionScreen):
    ...
    def perform_inference(self, window_data):
        """
        Performs inference using the loaded Keras model to classify pose form.
        Returns a dict with 'action' and 'feedback' keys.
        """
        ...
        if sequence_flattened.shape[0] < SEQUENCE_LENGTH:
            padding = np.zeros(
                (SEQUENCE_LENGTH - sequence_flattened.shape[0], NUM_LANDMARKS * NUM_FEATURES)
            )
